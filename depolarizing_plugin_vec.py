...
class DepolarizingPluginVec(AbstractPlugin):
    ...
    def compile(self, batch, harware_specs):
        ...
        self.job_observable = job.observable
        ...
        job_copy.circuit.nbqbits = 2*self.nbqbits
        ...
        for op in job.circuit:
            ...
            if not gdef.matrix:
                ...
                if gname == "STATE_PREPARATION":
                    ...
                    op.qbits += [qb + self.nbqbits for qb in op.qbits]
            ...
            conj_op = deepcopy(op)
            conj_op.qbits = [qb+self.nbqbits for qb in op.qbits]
            ...
            if len(op.qbits) == 1 and self.prob_1qb > 0:
                depol1_op = Op(gate="depol1", qbits=[op.qbits[0], op.qbits[0]+self.nbqbits], type=0)
            ...
            if len(op.qbits) == 2 and self.prob_2qb > 0:
                depol2_op = Op(gate="depol2", qbits=[op.qbits[0], op.qbits[1],
                                                     op.qbits[0]+self.nbqbits, op.qbits[1]+self.nbqbits],
                               type=0)
    ...
    def post_process(self, batch_result):
        ...
        #print(result)
        rho_vec = np.zeros(4**self.nbqbits, np.complex128)
        for sample in result:
            rho_vec[sample.state.int] = sample.amplitude
            #print(sample.state.int, sample.amplitude)
        rho_vec = rho_vec.reshape(2**self.nbqbits, 2**self.nbqbits)
        ...
        if self.job_type == ProcessingType.SAMPLE or self.job_observable is None:
            if self.nbshots == 0:

                # tracing out some qubits
                if self.qubits != list(range(self.nbqbits)):
                    qubits_to_trace_out = list(range(self.nbqbits))
                ...
                res = Result()
                res.raw_data = []
                for int_state, val in enumerate(rho_vec.diagonal()):
            ...
            if self.nbshots == 0:
                O_mat = make_matrix(self.job_observable)
                res = np.trace(np.dot(rho_vec, O_mat))
                return BatchResult(results=[Result(value=res, error=None)])
            raise Exception("nbshots > 0 not yet implemented")
